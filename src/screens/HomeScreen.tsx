...
type FlowType = 'manual' | 'automatico';
...
const HomeScreen: React.FC<HomeScreenProps> = ({ navigation }) => {
  const { user } = useContext(AuthContext) as { user: { id: number; apelido?: string } };

  const [modalVisible, setModalVisible] = useState<boolean>(false);
  const handleSubmit = (data: { score: number; feedback: string }): void => {
  ...
  const [loadingSalas, setLoadingSalas] = useState<boolean>(false);
  ...
  const [showProcessedModal, setShowProcessedModal] = useState<boolean>(false);
  const [pastedText, setPastedText] = useState<string>('');
  const [tempPlayersData, setTempPlayersData] = useState<TempPlayer[]>([]);

  const genderCounts: GenderCounts = useMemo(
    () =>
      tempPlayersData.reduce(
        (acc, p) => {
          if (p.genero === 'M') acc.masculino++;
          if (p.genero === 'F') acc.feminino++;
          return acc;
        },
        { masculino: 0, feminino: 0 } as GenderCounts
      ),
    [tempPlayersData]
  );
  ...
  const fetchSalasAtivas = async (): Promise<void> => {
    setLoadingSalas(true);
    try {
      const response = await api.get<{ salas: Sala[] }>('/api/lobby/me');
  ...
  const handleEnterSala = async (): Promise<void> => {
    if (!salaId.trim()) {
  ...
  const parsePlayerList = (text: string): string[] => {
    const lines = text.split('\n');
    const players: string[] = [];
    lines.forEach((line) => {
      const trimmed = line.trim();
      if (trimmed.includes('✅')) {
        let nome = trimmed.replace(/^\d+\.\s*/, '').replace(/✅/g, '').trim();
  ...
  const handleGenderSelect = (player: TempPlayer, gender: 'M' | 'F'): void => {
    setTempPlayersData((prev) =>
      prev.map((p) => (p.nome === player.nome ? { ...p, genero: gender } : p))
    );
  };

  const handleConfirmProcessedPlayers = (): void => {
  ...
  const handleSelectFlow = (flowType: FlowType): void => {
    setShowFlowModal(false);
    setShowPasteModal(false);
    setShowProcessedModal(false);
    if (flowType === 'manual') {
  ...
  const renderPartidaCard = (partida: Sala): JSX.Element => {
    ...
    return (
      ...
      >
        ...
        <View style={styles.partidaInfo}>
          ...
          <Text style={styles.partidaText}>{dataJogo.format('dddd, DD [de] MMMM')}</Text>
        </View>
        <View style={styles.partidaInfo}>
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView style={styles.container}>
        ...
        {/* BANNER */}
        <ImageBackground source={require('../../assets/images/banners/partida.png')} style={styles.bannerContainer} imageStyle={styles.bannerImageStyle} resizeMode="cover" />
        ...
        <View style={styles.nextMatchesHeader}>
         ...
         <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.partidasContainer}>
          {salasAtivas.slice(0, 5).map(renderPartidaCard)} {/* APENAS AS 5 ÚLTIMAS PARTIDAS */}
        </ScrollView>
      </ScrollView>
      ...
      {/* ... todos os modais conforme código original ... */}
